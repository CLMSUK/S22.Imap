...
namespace S22.Imap.Tests {
	...
	public class MessageBuilderTest {
		...
		public void BuildMessageFromHeader() {
            ...
            MailMessage m = MessageBuilder.FromHeader(Resources.MailHeader);
			Assert.AreEqual<string>(Resources.MailSubject, m.Subject);
			Assert.AreEqual<string>(Resources.MailSubjectEncoding, m.SubjectEncoding.BodyName);
			Assert.AreEqual<string>(Resources.MailFromName, m.From.DisplayName);
			Assert.AreEqual<string>(Resources.MailFromAddress, m.From.Address);
			Assert.AreEqual<MailPriority>(MailPriority.High, m.Priority);
			Assert.AreEqual<string>(Resources.MailXMimeOLE, m.Headers["X-MimeOLE"]);
		}
		...
		public void MessageWithAttachment() {
			MailMessage m = MessageBuilder.FromMIME822(Resources.MailWithZipAttachment);
			...
			Assert.AreEqual<string>(Resources.AttachmentName, m.Attachments[0].Name);
			using (var sr = new BinaryReader(m.Attachments[0].ContentStream)) {
				byte[] constructed = sr.ReadBytes((int) sr.BaseStream.Length);
				Assert.AreEqual<int>(
					Resources.ZipAttachment.Length,
					constructed.Length);
				Assert.IsTrue(Resources.ZipAttachment.SequenceEqual(constructed));
			}
		}
		...
		public void MessageWithQuotedPrintables() {
			...
			MailMessage m = MessageBuilder.FromMIME822(
				Resources.MailWithQuotedPrintables.
				Replace("\r\n", "\n").Replace("\n", "\r\n"));
			// Ensure the constructed body is identical to our resource string.
			Assert.AreEqual<string>(Resources.QuotedPrintableText.
				Replace("\r\n", "\n").Replace("\n", "\r\n"), m.Body);
		}
		...
		public void MessageWithMultipleParts() {
			MailMessage m = MessageBuilder.FromMIME822(Resources.MailWithMultipleParts);
			...
			using (var sr = new BinaryReader(m.Attachments[0].ContentStream)) {
				...
				Assert.AreEqual<int>(Resources.GifContent.Length, gif.Length);
				Assert.IsTrue(Resources.GifContent.SequenceEqual(gif));
			}
			using (var sr = new BinaryReader(m.Attachments[1].ContentStream)) {
				byte[] midi = sr.ReadBytes((int) sr.BaseStream.Length);
				Assert.AreEqual<int>(Resources.MidiContent.Length, midi.Length);
				Assert.IsTrue(Resources.MidiContent.SequenceEqual(midi));
			}
		}
		...
		public void MessageWithMultipartMixedAttachment() {
			...
			MailMessage m = MessageBuilder.FromMIME822(Resources.MailWithoutDisposition);
		...
		public void MessageWithInvalidFromHeader() {
			MailMessage m = MessageBuilder.FromMIME822(Resources.MailWithInvalidFromHeader);
		...
		public void MessageWithEncodedAttachmentName() {
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            MailMessage m = MessageBuilder.FromMIME822(
				Resources.MailWithEncodedAttachmentName);
			Assert.AreEqual<int>(1, m.Attachments.Count);
		...
		public void MessageWithRfc2231Headers() {
			MailMessage m = MessageBuilder.FromMIME822(Resources.MailWithRfc2231Headers);
			...
			m = MessageBuilder.FromMIME822(Resources.MailWithRfc2231Headers2);
			Assert.AreEqual<int>(1, m.Attachments.Count);
		...
		public void MessageWithMIMEComments() {
			MailMessage m = MessageBuilder.FromMIME822(Resources.MailWithMIMEComments);
